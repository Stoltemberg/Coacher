...
class VoiceAssistant:
    ...
    def __init__(self, callback_log=None):
        ...
        self.enabled = True
        self.volume = 80
        self.voice_personality = normalize_voice_personality(
            self._env("COACHER_VOICE_PERSONALITY", "standard")
        )
    ...
    def set_voice_personality(self, personality):
        self.voice_personality = normalize_voice_personality(personality)
    ...
    def _speech_worker(self):
        while True:
            ...
            # 1. Check personality override
            voice_profile = resolve_voice_personality(self.voice_personality)

            if voice_profile["engine"] == "edge_tts":
                try:
                    self._speak_with_standard_voice(final_text)
            ...
            # 2. Try Premium Voice (ElevenLabs)
            if voice_profile["engine"] == "elevenlabs" and self.tts_ready and self.audio_ready:
                try:
                    ...
                    if not os.path.exists(cache_path):
                        if (
                            ...
                            if not self._warned_tts_budget:
                                print("[Voz] Orcamento de TTS atingido. Usando fallback.")
                                self._warned_tts_budget = True
                            raise Exception("Budget exceeded")

                        self._generate_elevenlabs_audio(final_text, cache_path)
                    ...
                    if self.tts_ready:
                        print(f"[Voz] ElevenLabs falhou ({e}). Usando fallback...")
            
            # 3. Fallback to Standard Voice (SAPI/Edge)
            if not self._warned_missing_tts_config and not self.tts_ready:
                print("[Voz] ElevenLabs nao configurado. Usando fallback para voz do sistema.")
                self._warned_missing_tts_config = True

            try:
                self._speak_with_standard_voice(final_text)
            except Exception as e:
                print(f"[Voz] Falha critica no mecanismo de voz: {e}")
            finally:
                for _ in collected_items:
                    self.speech_queue.task_done()

    def _generate_elevenlabs_audio(self, text, cache_path):
        """Call ElevenLabs TTS API and write the resulting MP3 to *cache_path*."""
        ...
        self.tts_session_chars_used += len(text)
