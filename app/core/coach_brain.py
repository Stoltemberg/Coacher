...
class CoachBrain:
    ...
    def _get_counter_item_suggestion(self, need_type):
        ...
        picks = self.knowledge._item_picks(champ, need_type, limit=1)
        return picks[0] if picks else None

    def _get_qss_targets(self):
        """Retorna nomes de inimigos que têm supressão (Malzahar, Skarner, etc)."""
        suppress_champs = {"Malzahar", "Skarner", "Warwick", "Mordekaiser"}
        targets = []
        for name, team in self.player_to_team.items():
            if team != self.our_team:
                champ = self.player_to_champ.get(name, "")
                if champ in suppress_champs:
    ...
    def update_players(self, players_data):
        ...
        if (
            ...
            build_tip = None
            spike_tip = None
            # Compute item recommendations for this champion
            item_recs = {}
            if self.knowledge and self.current_champion_name:
                item_recs = self.knowledge.recommend_items_for_champion(self.current_champion_name)
            ...
            if not build_tip:
                build_tip = self.voice.comp_build_adjustment(self.current_champion_name, scoped_enemy_names, item_recs=item_recs)
            if build_tip:
        ...
        if current_time - self.last_item_check > self.settings.item_check_interval: 
            ...
            for p in players_data:
                ...
                if p.get("team") != self.our_team:
                    ...
                    # QSS / Cimitarra against suppression champions
                    if "qss" not in self.advised_items and self.game_time >= 600:
                        qss_targets = self._get_qss_targets()
                        if qss_targets:
                            target_names = " e ".join(qss_targets)
                            self._announce(
                                self.voice.qss_warning(target_names) if hasattr(self.voice, "qss_warning")
                                else f"Tem {target_names} do outro lado. Pensa em Cimitarra Prateada ou Purificação antes de entrar em fight.",
                                "negative",
                                category="item_advice",
                            )
                            self.advised_items.add("qss")
                            return
